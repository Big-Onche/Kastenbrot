...
namespace acoustics
{
    ...
    struct ChunkAcousticGrid
    {
        int x, y;
        ivec runtimeorigin;
        vector<AcousticCell> cells;

        ChunkAcousticGrid(int x = 0, int y = 0, const ivec &runtimeorigin = ivec(0, 0, 0))
    ...
    static bool getAcousticFile(const char *mname, char *acgname)
    {
        ...
        const char *mapname = strrchr(mname, '/');
        mapname = mapname ? mapname + 1 : mname;
        string cleanname;
        copystring(cleanname, mapname);
        if(char *extension = strrchr(cleanname, '.')) *extension = '\0';
        nformatstring(acgname, MAXSTRLEN, "packages/map/%s.acg", cleanname);
        path(acgname);
        return true;
    }
    ...
    {
        int size = chunkAcousticGrids.empty() ? max(soundacousticcellsize, 16) : WORLD_SECTION_SIZE;
        return ivec(int(floorf(o.x/size)), int(floorf(o.y/size)), int(floorf(o.z/size)));
    }

    static vec acousticCellCenter(const ivec &coord)
    {
        float size = chunkAcousticGrids.empty() ? max(soundacousticcellsize, 16) : WORLD_SECTION_SIZE;
    ...
    static void rebuildChunkAcousticGrid()
    {
        ...
        acousticRegionPortalEdges.setsize(0);
        acousticCellLookup.clear();
        loopv(chunkAcousticGrids)
        {
            ChunkAcousticGrid &grid = *chunkAcousticGrids[i];
            loopvj(grid.cells)
            {
                AcousticCell cell = grid.cells[j];
                cell.origin.add(vec(grid.runtimeorigin));
                ...
                int index = acousticCells.length();
                acousticCells.add(cell);
                acousticCellLookup[cell.coord] = index;
            }
        }
    ...
    static void chunkAcousticPutUint(vector<uchar> &data, uint value)
    {
        loopi(4) data.add(uchar(value >> (i*8)));
    }

    static bool chunkAcousticGetUint(const uchar *&position, const uchar *end, uint &value)
    {
    ...
    bool bakeChunkAcoustics(const worldsectionrenderdata &renderdata, int chunkx, int chunky, vector<uchar> &data, SDL_atomic_t *cancelled)
    {
        data.setsize(0);
        data.put((const uchar *)"KCAG", 4);
        chunkAcousticPutUint(data, 1);
        chunkAcousticPutUint(data, uint(chunkx));
        chunkAcousticPutUint(data, uint(chunky));
        chunkAcousticPutUint(data, WORLD_SECTION_SIZE);
        int countoffset = data.length();
        chunkAcousticPutUint(data, 0);
        uint count = 0;
        loop(section, WORLD_SECTION_LAYERS)
        {
            if(cancelled && SDL_AtomicGet(cancelled)) { data.setsize(0); return false; }
    ...
    bool installChunkAcoustics(int chunkx, int chunky, const ivec &runtimeorigin, const vector<uchar> &data)
    {
        ...
        if(!chunkAcousticGetUint(position, end, version) || version != 1 ||
           !chunkAcousticGetUint(position, end, storedx) || int(storedx) != chunkx ||
           !chunkAcousticGetUint(position, end, storedy) || int(storedy) != chunky ||
           !chunkAcousticGetUint(position, end, cellsize) || cellsize != WORLD_SECTION_SIZE ||
        ...
        loopi(int(count))
        {
            ...
            position += 4;
            if(x >= WORLD_SECTION_COLUMNS || y >= WORLD_SECTION_COLUMNS || z >= WORLD_SECTION_LAYERS ||
               flags & ~(SECTION_EXTERIOR | SECTION_INTERIOR | SECTION_CAVE_ENTRANCE | SECTION_WATER | SECTION_FULLY_SOLID | SECTION_NO_RENDER))
            {
                delete grid;
                return false;
            }
            ...
            cell.outdoorRatio = exterior ? (interior ? 0.5f : 1.0f) : 0.0f;
            cell.outdoorConnectivity = cell.skyOpenness = cell.outdoorRatio;
            cell.openness = exterior ? 0.85f : cave ? 0.35f : 0.25f;
            cell.confidence = cell.boundary ? 0.72f : 0.90f;
            cell.medianDistance = unitsToMeters(WORLD_SECTION_SIZE*0.5f);
            cell.medianHorizontalDistance = cell.medianDistance;
            cell.reverbGain = exterior ? 0.20f : cave ? 0.90f : 0.65f;
            cell.reflection = exterior ? 0.15f : cave ? 0.90f : 0.65f;
            cell.muffleOpen = exterior ? 0.95f : cave ? 0.45f : 0.65f;
            if(cave) cell.presetScores[AP_CAVE] = 1.0f;
            else if(exterior && interior) cell.presetScores[AP_COURTYARD] = 1.0f;
            else if(exterior) cell.presetScores[AP_OPENOUTDOOR] = 1.0f;
    ...
    void rebaseChunkAcoustics(float shiftx, float shifty)
    {
        ...
        loopv(chunkAcousticGrids)
        {
            chunkAcousticGrids[i]->runtimeorigin.x -= int(shiftx);
            chunkAcousticGrids[i]->runtimeorigin.y -= int(shifty);
        }
        rebuildChunkAcousticGrid();
    }
    ...
    {
        chunkAcousticGrids.deletecontents();
        clearAcousticGrid();
    }
    ...
    static bool acousticAStarSegmentPassable(const vec &from, const vec &to, bool streamed = false)
    {
        ...
        ray.div(len);
        if(acousticRaycube(from, ray, len, RAY_POLY) < len*0.96f) return false;
    ...
    static bool findAcousticAStarPath(const vec &loc, float dist, float directOcclusion, AcousticAStarResult &result, bool streamed = false)
    {
        ...
        if(streamed && (!acousticAStarSegmentPassable(loc, acousticAStarCellCenter2D(sourcecoord, z, cellsize), true) ||
                        !acousticAStarSegmentPassable(acousticAStarCellCenter2D(targetcoord, z, cellsize), camera1->o, true))) return false;
        ...
        while(!queue.empty())
        {
            ...
            loopi(sizeof(dirs)/sizeof(dirs[0]))
            {
                ...
                if(!acousticAStarSegmentPassable(curCenter, nextCenter, streamed)) continue;
    ...
    {
        return soundpointocclusion(from, to, [](const vec &start, const vec &direction, float length)
        {
            return acousticRaycube(start, direction, length, RAY_POLY);
        });
    }
    ...
    void acousticAmbientSource(const vec &loc, float dist, float &volf, float &gainhf, float &reverbSend, AcousticSourceInfo &info)
    {
        ...
        // Open neighboring baked cells are not alternative positions for these sources.
        float occlusion = soundpointocclusion(loc, camera1->o, [](const vec &start, const vec &direction, float length)
        {
            float hit = acousticRaycube(start, direction, length, RAY_POLY);
        ...
        if(!findAcousticAStarPath(loc, dist, occlusion, path, true)) return;
        ...
        if(acousticProbe.baked)
            reverbSend = max(reverbSend, clamp(acousticProbe.reverbGain * soundacousticreverb *
                                              (0.35f + path.complexity * 0.25f), 0.0f, 1.0f));
    }
