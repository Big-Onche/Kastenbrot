...
extern Texture *textureload(const char *name, int clamp = 0, bool mipit = true, bool msg = true, bool unfiltered = false);
extern Texture *loadworlditemtexture(Slot &slot);
...
extern void setfog(int fogmat, float below, float blend, int abovemat);
...
extern float sampleworldskyheight(int x, int y);
...
extern void rendertransparent(int fogmat = MAT_AIR, float fogbelow = 0, int abovemat = MAT_AIR, bool cloudsbeforeliquid = false);
...
extern void setupworldlodwater(bool detailed = false, bool below = false);
extern bool findworldlodwater(float &sx1, float &sy1, float &sx2, float &sy2);
extern void renderworldlodwater(bool mask = false);
...
extern bool haswatergeometry(int material);
extern void renderwatergeometry(int material, bool mask = false, int side = 0);
...
extern void invalidatewatergeometry(const ivec &minimum, const ivec &maximum);
extern float getwatergeometryoffset();
extern bool haswaterfallgeometry(int material);
extern void renderwaterfallgeometry(int material, bool mask = false);
