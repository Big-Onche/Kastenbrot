...
FVARR(aoradius, 0, 9, 256);
FVAR(aocutoff, 0, 2.0f, 1e3f);
FVARR(aodark, 1e-3f, 13.0f, 1e3f);
...
VARF(rhfarplane, 64, 4096, 16384, clearradiancehintscache());
...
static shadowmapinfo *addshadowmap(ushort x, ushort y, int size, int &idx, int light, shadowcacheval *cached = NULL)
{
...
void renderlights(float bsx1 = -1, float bsy1 = -1, float bsx2 = 1, float bsy2 = 1, const uint *tilemask = NULL, int stencilmask = 0, int msaapass = 0, bool transparent = false)
{
    ...
    bool sunpass = !lighttilebatch || drawtex == DRAWTEX_MINIMAP || (csm.rendered && batchsunlight <= (useradiancehints() ? 1 : 0));
    if(sunpass)
    {
        if(depthtestlights && depth) { glDisable(GL_DEPTH_TEST); depth = false; }
...
void rendercsmshadowmaps()
{
    ...
    // Polygon slope is already depth change per shadow texel; no cascade or PCF multiplier.
    if(csmslopebias)
    {
    ...
    loopi(csmsplits)
    {
        ...
        if(csm.colortex && (shadowtransparent&(1<<i)))
        {
            setupshadowtransparent();
            renderalphashadow(false);
