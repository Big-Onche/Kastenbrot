...
enum // cube empty-space materials
{
    ...
    // Pane placement metadata. Material rendering masks these bits out, so they
    // do not prevent adjacent panes of the same quality from being merged.
    MAT_GLASS_PANE_AXIS = 64 << MATF_FLAG_SHIFT,
    MAT_GLASS_PANE = 128 << MATF_FLAG_SHIFT
};
...
enum
{
    ...
    RAY_CLIPMAT = 16, RAY_SKIPFIRST = 32, RAY_EDITMAT = 64, RAY_PASS = 128, RAY_LIQUIDMAT = 256
};
...
#ifdef WIN32
#define PERSISTENT_ULL_FORMAT "%I64u"
#else
#define PERSISTENT_ULL_FORMAT "%llu"
#endif
inline void putpersistentid(packetbuf &buffer, ullong id) { loopi(8) buffer.put(uchar(id >> (i * 8))); }
inline ullong getpersistentid(ucharbuf &buffer)
{
    ullong id = 0;
    loopi(8) id |= ullong(buffer.get()) << (i * 8);
    return id;
}
...
extern int getworldcubebytextures(const ushort *textures);
extern int getworldcubeindex(int slot);
extern int getworldcubeindexat(const ivec &position, int orient);
extern ullong getworldcubepersistentid(int index);
...
extern bool isworldcubeoccupiedat(const ivec &position);
extern bool getworldcubeacceptspaneconnection(int index);
...
extern int getworldcubeidindex(const char *id);
...
extern int getworldscatterpersistentindex(ullong id, bool warn = true);
...
extern bool getworldplaceableblockcollision(int index);
...
extern ullong getinventoryitempersistentid(int index);
extern int getinventoryitempersistentindex(ullong id, bool warn = true);
...
extern bool getinventoryitemheldflipx(int index);
extern bool getinventoryitemheldflipy(int index);
extern float getinventoryitemheldsize(int index);
extern int getinventoryitemindex(const char *id);
extern int numwornslots();
extern int getwornslotindex(const char *id);
extern const char *getwornslotid(int index);
extern const char *getwornslotname(int index);
extern bool getwornslotmirrored(int index);
extern bool getinventoryitemequipmentcompatible(int item, int wornslot);
extern int getinventoryitemequipmentcapacity(int item);
...
extern float getwaterimmersion(const physent *pl);
...
extern void waterterrainchanged(const ivec &position);
...
extern void modeltagpositions(const char *mdl, const char * const *tags, vec *positions, bool *found, int numtags, const vec &o, float yaw = 0, float pitch = 0, float roll = 0, float size = 1, int anim = ANIM_MAPMODEL | ANIM_LOOP);
extern bool modeltagposition(const char *mdl, const char *tag, vec &position, const vec &o, float yaw = 0, float pitch = 0, float roll = 0, float size = 1, int anim = ANIM_MAPMODEL | ANIM_LOOP);
