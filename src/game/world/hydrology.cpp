...
namespace game
{
    // Drainage planning and caches belong to one worldgenerator/job.
    ...
    struct worldhydrology
    {
        ...
        float rawhead(const key &k)
        {
            ...
            return max(float(generator.settings.sealevel),
                       (terrain(x, y) * 2 + terrain(x - 8, y) + terrain(x + 8, y) + terrain(x, y - 8) + terrain(x, y + 8)) / 6 - 1);
        }
        ...
        lake regionlake(const key &region)
        {
            ...
            if(selection > -0.15f)
            {
                ...
                float best = 1e9f;
                for(int y = 48; y <= 208; y += 40)
                    for(int x = 48; x <= 208; x += 40)
                    {
                ...
                const float radius = (h % 20 == 0) ? 40 + float((h >> 8) % 45) : (h % 4 == 0) ? 12 + float((h >> 8) % 19) : 5 + float((h >> 8) % 6);
        ...
        const route &regionroute(const key &region)
        {
            ...
            key source(region.first * (SOURCEREGION / STEP) + 2 + int(h % 4), region.second * (SOURCEREGION / STEP) + 2 + int((h >> 8) % 4));
            ...
            loopi(8)
            {
                ...
                if(terrain(int(start.x + cosf(angle) * 48), int(start.y + sinf(angle) * 48)) <= generator.settings.sealevel) inlandsource = false;
            }
            ...
            if(outlet || (inlandsource && elevation >= 12 && slope > 1 && float((h >> 16) & 255) / 255.0f < min(0.85f, elevation / 80.0f)))
            {
                ...
                loop(step, MAXSTEPS)
                {
                    ...
                    if(n.head <= generator.settings.sealevel)
                    {
                        complete = true;
                        break;
                    }
                ...
                if(!complete || result.nodes.size() < 4)
                {
                    result.nodes.clear();
                    result.terminal.valid = false;
                }
            }
        ...
        tile &gettile(const key &position)
        {
            ...
            for(int ry = divide(oy - 160, REGION); ry <= divide(oy + TILE + 160, REGION); ++ry)
                for(int rx = divide(ox - 160, REGION); rx <= divide(ox + TILE + 160, REGION); ++rx)
                {
                    ...
                    if(l.valid && l.x + l.rx * 1.5f + 32 >= ox && l.x - l.rx * 1.5f - 32 <= ox + TILE && l.y + l.ry * 1.5f + 32 >= oy &&
                       l.y - l.ry * 1.5f - 32 <= oy + TILE)
                        result.lakes.push_back(l);
                }
            for(int ry = divide(oy - halo, SOURCEREGION); ry <= divide(oy + TILE + halo, SOURCEREGION); ++ry)
                for(int rx = divide(ox - halo, SOURCEREGION); rx <= divide(ox + TILE + halo, SOURCEREGION); ++rx)
                {
                    ...
                    if(r.terminal.valid && !r.nodes.empty() && terminalkeys.insert(r.nodes.back()).second)
                    {
                        ...
                        if(l.x + l.rx * 1.5f + 32 >= ox && l.x - l.rx * 1.5f - 32 <= ox + TILE && l.y + l.ry * 1.5f + 32 >= oy &&
                           l.y - l.ry * 1.5f - 32 <= oy + TILE)
                            result.lakes.push_back(l);
                    }
        ...
        float moisture(float x, float y, float z)
        {
            ...
            for(size_t i = 0; i < t.lakes.size(); ++i)
            {
                ...
                const float radius = l.radius(x, y), dx = x - l.x, dy = y - l.y,
                            ...
                            horizontal = 1.0f - smoothstep(0.0f, 24.0f, distance), vertical = 1.0f - smoothstep(8.0f, 64.0f, fabsf(z - l.level));
            ...
            for(size_t i = 0; i < t.channels.size(); ++i)
            {
                ...
                const float u = clamp(((x - c.a.x) * vx + (y - c.a.y) * vy) / (length * length), 0.0f, 1.0f), bend = sinf(u * M_PI) * c.bend,
                            ...
                            level = c.a.head + (c.b.head - c.a.head) * u, horizontal = 1.0f - smoothstep(width, width + 24.0f, distance),
        ...
        worldwatersample sample(int x, int y)
        {
            ...
            for(size_t i = 0; i < t.lakes.size(); ++i)
            {
                ...
                const float blend = 1 - smoothstep(0.75f, 1.35f, r), floor = l.level - 1 - l.depth * max(0.0f, 1 - r * r),
            ...
            if(!inlake)
                for(size_t i = 0; i < t.channels.size(); ++i)
                {
                    ...
                    const float vx = c.b.x - c.a.x, vy = c.b.y - c.a.y, length = sqrtf(vx * vx + vy * vy),
                                ...
                                bend = sinf(u * M_PI) * c.bend, px = c.a.x + vx * u - vy / length * bend, py = c.a.y + vy * u + vx / length * bend,
                                distance = sqrtf((x + 0.5f - px) * (x + 0.5f - px) + (y + 0.5f - py) * (y + 0.5f - py)), size = sqrtf(c.flow),
                                width = min(16.0f, 0.75f + size * 0.9f) * (1 + 0.12f * sinf((x + y) * 0.035f)), depth = min(10.0f, 2.0f + size),
                                level = c.a.head + (c.b.head - c.a.head) * u, bankwidth = width + max(3.0f, depth * 2.5f), r = distance / bankwidth;
                    if(r >= 1 || base < sea) continue;
                    const float bankblend = 1 - smoothstep(width, bankwidth, distance), slope = fabsf(c.a.head - c.b.head) / max(length, 1.0f),
                                // Leave clearance below the smoothed surface on steep reaches, with a shallow bank apron.
                        clearance = 1.0f + min(4.0f, slope * 1.5f),
                                temperature = generator.environmentclimate.gettemperature(
                                    vec(x * float(worldclimate::BLOCK_UNITS), y * float(worldclimate::BLOCK_UNITS),
                                        worldclimate::GROUND_UNITS + base * float(worldclimate::BLOCK_UNITS))),
    ...
    float worldgenerator::gethumidity(const vec &worldpos) const
    {
        const float x = worldpos.x / worldclimate::BLOCK_UNITS, y = worldpos.y / worldclimate::BLOCK_UNITS,
                    z = (worldpos.z - worldclimate::GROUND_UNITS) / worldclimate::BLOCK_UNITS, noisex = x + 10000.5f, noisey = y - 10000.5f,
                    continental = samplecontinental(*this, noisex, noisey), coastdistance = samplecoastdistance(*this, noisex, noisey, continental),
    ...
    float worldgenerator::freshwatermoisture(float x, float y, float z) const
    {
        if(!hydrology) hydrology = new worldhydrology(*this);
        return hydrology->moisture(x, y, z);
    }
