...
namespace game
{
    ...
    {
        setupnoise(biomeintrusions, seed ^ 0x19B5A731, 0.006f, 2, 0.35f);
    ...
    static float blenddesertcoverage(float coverage, float relativeheight, float slope, float freshwater)
    {
    ...
    int worldgenerator::surfacematerial(int x, int y, int height) const
    {
        ...
        const BiomeSample soil = sampleClimateBiome(environmentclimate.getregionaltemperature(position), sample.humidity);
        return sandcoverage(x, y, height, soil) > sandthreshold(float(x), float(y)) ? WORLD_BIOME_DESERT : WORLD_BIOME_PLAINS;
    }
